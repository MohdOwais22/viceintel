function normalizeProfile(doc: any): any {
  ...
  return {
    ...
    vcBalance: typeof p.vcBalance === 'number' ? p.vcBalance : 0,
    dailyStreak: streak,
    streakCount: streak,
    rewardStreak: streak,
    isAdmin: isL4,
    ...
    vipExpires: p.vipExpires || (isL4 ? 'Lifetime' : isL3 ? 'Staff Account' : isL2 ? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] : 'Expired'),
