...
🎖️ **Badges:** ${badgesStr}`;
  ...
  // -------------------------------------------------------------------
  // AUTOMATED BACKGROUND CRON JOBS (ALL AUTOMATIC TRIGGERS STOPPED)
  // -------------------------------------------------------------------
  // All automatic background cron intervals (pSEO crawler, FiveM traffic sync,
  // VIP expiry check, squad cleanup, challenges payout, pSEO merge/prune,
  // and Discord role sync) have been stopped per user request.
  // All functions remain available for on-demand execution via the Admin Panel
  // or explicit authenticated webhooks.
  console.log('[Cron Engine] Notice: All automatic scheduled background cron jobs are STOPPED (Manual on-demand triggers only).');
...
`;
  ...
  // Automated Discord Role Synchronization (STOPPED / MANUAL ON-DEMAND ONLY)
  // All automatic background cron executions stopped per user request.
  // Manual trigger endpoint: POST /api/cron/discord-role-sync or POST /api/discord/sync-roles
