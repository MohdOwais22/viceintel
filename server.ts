 ...
 */
async function cleanAndPrunePseoArticles(): Promise<{
  mergedCount: number;
  prunedCount: number;
 ...
 */
async function runMidnightPseoGenerator(queryOverride?: string) {
...
🎖️ **Badges:** ${badgesStr}`;
  ...
  async function runVipExpiryCheckServer(forceBypassSentCheck: boolean = false) {
    const alertLogs: any[] = [];
    let scannedCount = 0;
