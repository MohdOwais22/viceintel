...
### 9. Automated Web Search & pSEO News Spider Engine
- **Background Cron Scheduler**: `server.ts` executes a recurring interval check every 15 minutes (`15 * 60 * 1000`) when `AUTO_PSEO_ENABLED !== 'false'` to crawl real-time intelligence and index fresh articles.
- **Automated 12-Hour Smart Merge & Prune Cron**: `server.ts` executes a scheduled pass every 12 hours (`12 * 60 * 60 * 1000`) calling `cleanAndPrunePseoArticles()` to consolidate overlapping topics into rich deep-dive guides and prune expired (>30 days old) news entries across server memory and Firestore `pseoArticles`.
...
- **Rest & Webhook Endpoints**:
  ...
  - `POST /api/seo/merge-and-prune` / `GET /api/cron/merge-prune`: Triggers smart consolidation & 30-day retention prune on-demand.
- **Interactive Knowledge Hub**: `GtaSeoKnowledgeHub.tsx` loads generated pages live with an active index counter (`X Articles Indexed`) and includes `⚡ Run Live Crawler & Fetch Articles` and `Consolidate & Prune Stale Articles` trigger buttons.
